...
void pfdCompress(const vector<unsigned>& invertedIndex, vector<unsigned>& result, int& idx)
{
	...
	unsigned b = max((int)ceil(log2(iCopy[(unsigned)((normalLen-1) * PFOR_THRESHOLD)] + 1)),1);
...
void pfdDecompressAll(const vector<unsigned>& compressedLists, vector<vector<unsigned>>& result)
{
	...
	{
		curList = pfdDecompress(compressedLists, idx);
